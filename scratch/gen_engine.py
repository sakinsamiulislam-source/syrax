...
class ProductAwareResolver:
    ...
    def parse_query_product(cls, query: str) -> Tuple[Optional[str], MarketProduct, bool, Optional[MarketProduct], bool]:
        ...
        is_comparison = bool(
            re.search(r'(?:compare|vs|versus|better|difference between)', q) and
...
"
            f"| :--- | :--- | :--- | :--- |
"
            f"| **Live Price** | `${spot_p:,.2f}` | `${fut_p:,.2f}` | Basis: `{basis:+.2f} USD` ({basis_pct:+.4f}%) |
"
...
"
        ...
        return {
            ...
            "headline": f"⚖️ {base} Spot vs Futures Comparison (Basis: {basis:+.2f} USD)",
